...
public class BikeController {
    ...
    public ResponseEntity<BikeResponse> create(@Valid @RequestBody CreateBikeRequest request, UriComponentsBuilder uriBuilder) {
        Bike bike = bikeService.create(new CreateBikeCommand(request.serialNumber(), request.type(), request.stationId()));
    ...
    @GetMapping("/{bikeId}")
    public ResponseEntity<BikeResponse> getBike(@PathVariable UUID bikeId) {
    ...
    @GetMapping
    public ResponseEntity<PagedResponse<BikeResponse>> getBikes(@RequestParam(name = "stationId", required = false) UUID stationId, @RequestParam(name = "status", required = false) String status, @PageableDefault(sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
    ...
    public ResponseEntity<BikeReservationResponse> reserveBike(@PathVariable UUID bikeId, @RequestHeader(IDEMPOTENCY_HEADER) UUID operationId) {
        BikeReservationResult result = bikeReservationCommandService.reserveBike(bikeId, operationId);
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(new BikeReservationResponse(result.id(), result.stationId()));
    }

    @PostMapping("/{bikeId}/release")
    public ResponseEntity<BikeResponse> releaseBike(@PathVariable UUID bikeId, @RequestHeader(IDEMPOTENCY_HEADER) UUID operationId) {
        return ResponseEntity.status(HttpStatus.ACCEPTED).body(mapper.toResponse(bikeReservationCommandService.releaseBike(bikeId, operationId)));
    }

    @PostMapping("/{bikeId}/maintenance")
    public ResponseEntity<BikeResponse> startMaintenance(@PathVariable UUID bikeId) {
    ...
    public ResponseEntity<BikeResponse> completeMaintenance(@PathVariable UUID bikeId) {
    ...
    public ResponseEntity<BikeResponse> retireBike(@PathVariable UUID bikeId) {
